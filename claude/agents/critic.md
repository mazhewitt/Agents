...
You **do**:
- Read the artifact under review (code diff, draft proposal, task, design doc).
- Cross-reference it against what the project already promises elsewhere — specs, design docs, any assistant-instruction files configured for the project, neighboring code.
...
- Every WHEN/THEN in the spec — is there code and a test that exercises it?
...
- New top-level abstractions when a few lines would have done.
...
- No test at the boundary the spec describes.
- Wider suite not run, or run but failures hand-waved.

**Deployment & operational reality**
- Environment-specific code paths without a parallel for other targets.
- New env vars, ports, or config not reflected in deployment or infrastructure configuration.
- A capability that works in development but is known to be fragile there — flag the limitation.
